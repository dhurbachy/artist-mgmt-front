...
import { useCreateArtist, useGetArtist, useUpdateArtist } from "../hooks/artist";
...
export default function CreateArtist({ open, onOpenChange, artistId }: CreateArtistProps) {
    ...
    // ✅ Fix 1: Populate form when artistData loads (editing mode)
    useEffect(() => {
        if (artistId && artistData) {
            setForm({
                ...
                first_release_year: artistData.first_release_year ?? new Date().getFullYear(),
    ...
    // ✅ Fix 2: Basic form validation
    const validate = () => {
        const newErrors: Record<string, string> = {};
        if (!form.name.trim()) newErrors.name = "Name is required";
        if (!form.dob) newErrors.dob = "Date of birth is required";
        if (form.first_release_year < 1900 || form.first_release_year > new Date().getFullYear()) {
    ...
    const handleSave = () => {
        ...
        if (artistId) {
            // ✅ Fix 3: Pass id alongside payload for updateArtist
            updateArtist(
                { id: artistId, ...form },
                {
                    onSuccess: () => {
                    ...
                    onError: (err: any) => {
                        setErrors(err.response?.data?.errors || { name: "Something went wrong" });
                    },
                }
            );
        } else {
